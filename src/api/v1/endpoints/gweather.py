...
def get_google_conditions(request: WeatherRequest):
    print(
        f"🌤️  [GOOGLE CONDITIONS] Fetching current weather conditions for coordinates: ({request.lat}, {request.lon})")
    try:
...
def get_google_forecast_hourly(request: WeatherRequest):
    print(
        f"⏰ [GOOGLE HOURLY] Fetching hourly forecast for coordinates: ({request.lat}, {request.lon})")
    try:
        coordinates = (request.lat, request.lon)
...
def get_google_forecast_daily(request: WeatherRequest):
    print(
        f"📅 [GOOGLE DAILY] Fetching daily forecast for coordinates: ({request.lat}, {request.lon})")
    try:
        coordinates = (request.lat, request.lon)
...
def get_google_history(request: WeatherRequest):
    print(
        f"📊 [GOOGLE HISTORY] Fetching weather history for coordinates: ({request.lat}, {request.lon})")
    try:
        coordinates = (request.lat, request.lon)
